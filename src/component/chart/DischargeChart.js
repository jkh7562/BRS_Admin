...
const DischargeChart = () => {
    ...
    useEffect(() => {
        const fetchAndProcess = async () => {
            try {
                ...
                // 분리(배출) 데이터만 필터링
                const dischargeData = raw.filter((entry) => {
                    const { boxLog } = entry
                    return boxLog && boxLog.type === "분리"
                })

                // 시간 단위별 데이터 집계
                const grouped = {}

                dischargeData.forEach((entry) => {
    ...
    return (
        <div>
            ...
            <BarChartContainer data={chartData} barColor="#FB7185" barName="배출량" />
        </div>
    )
}
